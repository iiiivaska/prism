...
/// macOS host (`swift test`) and the simulators. The reading itself is `DSSpec`: what used to be five regular
/// expressions over the file's text — one per block the suite needed — is the parsed document, which is also what
/// the binding tests read their matrices out of.
@Suite("Manifest and examples follow the specs (ADR-0006)")
struct DSComponentsContractTests {
    /// The checkout the specs are read from.
    static let repository = DSSpec.repository

    /// The spec file as written, for the checks that are about its prose rather than about a binding.
    static func spec(_ name: String) throws -> String {
        try DSSpec.component(name).text
    }

    nonisolated static let implemented = ["Surface", "Text", "Button", "Card", "Divider", "Icon"]
    ...
    func manifestDeclaresTheSpecVersionOnEverySupportedPlatform(_ name: String) throws {
        ...
        let version = try spec.specVersion
        let platforms = try spec.platforms
    ...
    func everySpecExampleHasAnExample(_ name: String) throws {
        let ids = try DSSpec.component(name).exampleIDs
        #expect(!ids.isEmpty)
    ...
    func everyExampleRendersInItsSchemes(_ name: String) throws {
        let declared = try DSSpec.component(name).exampleSchemes
...
struct DSExampleHandlerTests {
    ...
    @Test func everyCardExampleRendersTheActionItsSpecDeclares() throws {
        let declared = try DSSpec.component("Card").exampleActions
        let examples = DSExamples.all.filter { $0.component == "Card" }
        #expect(!examples.isEmpty)
        for example in examples {
    ...
    @Test func theDeclaredActionsAreReadBackFromTheSpec() throws {
        let sample = """
        ...
        notes:
        """
        #expect(try DSSpec.parsing("Sample", sample).exampleActions == ["a": "custom", "c": "none"])
    }
