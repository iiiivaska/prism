...
enum DSSnapshotMatrix {
    ...
    nonisolated static let components = ["Surface", "Text", "Button", "Card", "Divider", "Icon", "Badge", "IconButton", "Avatar", "Chip"]
...
struct DSSnapshotMatrixTests {
    /// 8 Surface, 6 Text, 7 Button, 7 Card, 6 Divider, 11 Icon, 10 Badge, 12 IconButton, 12 Avatar and 13 Chip examples;
    /// one Card example is light only; 3 Surface, 1 Text, 2 Card, 1 Divider, 2 Icon, 1 Badge, 1 IconButton, 3 Avatar and
    /// 4 Chip examples render glass — Avatar's two over the map because its circle is the glass chip there, and its one
    /// inside the glass Surface; Chip's two over the map and its one inside the vivid Surface because its pill is the
    /// glass chip there, and its one inside the glass Surface.
    ///
    ...
    /// Avatar 12×8 + 3×4 = 108, Chip 13×8 + 4×4 = 120.
    static let expectedCount = 872
