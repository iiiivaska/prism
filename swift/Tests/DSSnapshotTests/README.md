...
Divider, Icon, Badge, IconButton, Avatar and Chip across the accessibility matrix, the equal-width figures of ADR-0021 §5 as assertions, and **every check that reads a
...
| `DSSurfaceChipRenderTests.swift` | What the Surface module's glass chip draws (ADR-0036 §5, §6, rules 4, 6 and 7): over ADR-0036 F2's stripe fixture its in-bounds mirror reads the reflection model within a stated tolerance that tells it apart from the clamp and transparency models, with Surface's bleed reading the clamp model as the control; before the blur, the crop is mirrored about every edge and corner until the band is covered, pixel for pixel; over a white field that starts 12 pt inside the edge, the reflections add what the model says they add, which a band ending inside that margin would not; the mirror paints nothing outside its shape, a capsule; on the scheme's glass, on light glass and inside another glass chip, a chip draws the recipe's fill and edge and no mirror, since the pixels handed to it move no byte of the render, as they do on the page, the control; and its fallback paints `color.bg.surface.raised` over `color.bg.page` in both schemes. ADR-0037's fixture — a 32 pt circle that follows Avatar's `root.background` table inside a 140 × 40 host, over 2 pt red and blue stripes declared as a map — reads the sharp red stripe under the recipe's fill at the circle's centre inside a host that renders nothing (rule 1), where the circle alone on the map reads the blur, the control; and `color.bg.surface.raised` over `color.bg.page` inside a host that renders its own cell, `color.bg.surface.nested` (rule 2), in both schemes. It adds no baseline: Avatar and Chip, the components that draw a chip, are photographed through their own examples. iOS only. |
| `DSChipRenderTests.swift` | What a pressed Chip draws over media: over a black backdrop with one white band, declared as a map, the pressed pill still blurs the band — no sharp step between columns — and shows it within 1 pt of where the unpressed pill shows it, 24 pt from the pill's centre, where a mirror that scaled with the press would draw it off; the band moved 8 pt moves the reading with it, the control. iOS only. |
| `DSButtonRenderTests.swift` | What a Button draws: the ghost pill's missing rest fill, the disabled dim. iOS only. |
| `DSButtonReduceMotionTests.swift` | That the Reduce Motion press substitute is visible for every variant on the page and on vivid in both schemes (ADR-0023 §8.4), primary's pressed step included (ADR-0039), with no known issue. iOS only. |
...
| `DSButtonAccessibilityTreeTests.swift` | What a loading Button publishes to VoiceOver, read off the same tree with Divider's walk: the `loading` example is one button named `Saving, loading`, byte for byte the web's, which is the English `strings.Button.loading` filled with its label, and a template the app sets in `\.dsStrings` reaches the name while a button that is not loading keeps its label (Button.yaml behavior 3, ADR-0032). Skipped with the reason where the runtime has no automation switch. iOS only. |
...
| `DSChipAccessibilityTreeTests.swift` | What a Chip publishes to VoiceOver, read off the same tree with Divider's walk: every example is one button named by its `label` byte for byte as the web names it (the Cyrillic `russian-label` included), with no value and no hint, the selected trait exactly on the three filters that are on, not-enabled exactly on `disabled`, and for `removable` a second button after it named `Remove North yard`, in the app's `strings.Chip.remove` template; a filter that is off is still a button, a chip with no handler and no `isSelected` is text with no button trait, and the glyphs, the check and the decorative Avatar are never elements. Skipped with the reason where the runtime has no automation switch. iOS only. |
| `DSAvatarAccessibilityTreeTests.swift` | What an Avatar publishes to VoiceOver, read off the same tree with Divider's walk: every example with a name that is not decorative is one element with the image trait, named by that name byte for byte as the web names it (the Cyrillic `russian-initials` included), and `fallback-icon` and `decorative` are no element; an avatar with a blank name or marked decorative is no element either, one whose name has no letter still is, and neither the initials nor the glyph is ever an element of its own. Skipped with the reason where the runtime has no automation switch. iOS only. |
...
| `DSIconButtonReduceMotionTests.swift` | That an IconButton's press is visible under Reduce Motion for every variant and a selected circle, primary's and the selected circle's pressed step included (ADR-0039), with no known issue; and that danger's pressed overlay and the pressed step of primary and a selected circle show on every press, not only under Reduce Motion. iOS only. |
...
| Avatar | 12 (3 glass) | 12×8 + 3×4 = **108** |
| Chip | 13 (4 glass) | 13×8 + 4×4 = **120** |
| | **92** | **872** |

`DSSnapshotMatrixTests.theMatrixHasTheExpectedSizeAndUniqueNames` holds that 872, so adding an example is a deliberate
