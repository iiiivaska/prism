...
Divider, Icon, Badge, IconButton and Avatar across the accessibility matrix, the equal-width figures of ADR-0021 §5 as assertions, and **every check that reads a
...
`DSSnapshotMatrixTests.theMatrixHasTheExpectedSizeAndUniqueNames` holds that 752, so adding an example is a deliberate
