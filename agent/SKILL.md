...
- **On vivid**: a solid cell (a primary Button, the active segment) is `color.bg.fill.inverse-media` with `color.text.on-inverse-media`; rings and outlines are `color.border.on-media`. A pressed solid is one lightness step from its rest, `color.bg.fill.inverse-pressed`, or `color.bg.fill.inverse-media-pressed` on vivid, or knocked out `color.bg.fill.on-inverse-pressed` on inverse and `color.bg.fill.on-accent-pressed` on the lit tile, never an overlay laid on the solid (ADR-0039, ADR-0040). On the scheme's glass, rings are `color.border.on-glass-fill`, and the solid is `color.bg.fill.inverse` with `color.text.on-inverse`, ink on light glass and white on smoke, as on light glass: the white media solid is vivid's alone (ADR-0040 §1). The 1 px inner highlight of a vivid or glass surface is Surface's own (`color.edge.highlight`, `color.edge.raised`) — do not draw it yourself.
- **Charts follow the surface's published material**: solid → `color.chart.series.1…6` and the rest of `color.chart.*`; vivid → `color.chart.on-media.line|reference|grid`; glass → `color.chart.on-glass-fill.plot|line|reference|grid`. There is no chart family for `glassLight`, `inverse` or `accent`: a meter there draws its track and fill in the material's own tones, and a trend is said in words. A target line is its family's target role — `color.chart.target`, `color.chart.on-media.reference` or `color.chart.on-glass-fill.reference` — dashed with `stroke.target`, never a second series. Axis labels are `type.axis`; a gauge's stroke is `chart.gauge-stroke-ratio` of its diameter.
...
- **Accent fill**: on `color.bg.fill.accent` the primary tone is `color.text.on-accent` and secondary, tertiary and dimmed are all `color.text.on-accent-secondary`.
- **Inverse, the lit tile and light glass**: everything on them takes the material's foreground. On `color.bg.fill.inverse` every tone, ring and outline is `color.text.on-inverse`; on the lit tile quieter tones, rings and outlines are `color.text.on-accent-secondary`; on light glass every tone is `color.text.on-glass-light` and rings are `color.border.on-glass-fill`. On inverse and on the tile the one solid knocks out, filled with the material's foreground and carrying the material's fill (`color.text.on-inverse` under `color.bg.fill.inverse`, `color.text.on-accent` under `color.bg.fill.accent`), and nothing else draws a ground of its own (ADR-0040). In dark, `color.bg.tint.accent` fills chart windows and bands, never a card, and a `Card variant=tinted` is light only.
