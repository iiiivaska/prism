...
**The corpus predated this rule, and its misfits are not precedents.** Thirty boolean props in `spec/` were named before it: the imperative `show` (`ProgressBar.showValue`, now `showsValue`), bare adjectives and nouns (`HeroNumber.live`, now `isLive`; `LineChart.grid`, now `showsGrid`; the patterns' region switches, such as `DetailScreen.hero`, now `hasHero`, since a pattern has a region or lacks it), verbs for a behavior (`RingGauge.clampOverflow`, now `clampsOverflow`; `TextArea.autoGrow`, now `growsWithValue`), a second name for a meaning another spec already names (`AdaptiveShell.sidebarCollapsed`, now `isSidebarCollapsed`, beside the `Sidebar.isCollapsed` it forwards), and `Surface.selected`. `docs/roadmap.md` P4-D3 lists every one. Each was renamed by a change to its own spec: `Button.fullWidth` became `isFullWidth` with a `specVersion` bump and both stacks in Button 4, and the twenty-eight in specs nothing implements were renamed in place by P4-11. `Surface.selected` is the one still owed, and its rename is a `specVersion` bump on both stacks. A new prop follows the rule from its first commit, whatever a neighbour is called. `spec:validate` checks the form (`prop/boolean-name`): a boolean opens with `is`, `has`, `shows` or a behavior's own verb and says what after it, and no word after the verb is a negation. A behavior verb joins the closed list in `tools/spec/config.ts` in the change that first uses it, since a noun can end in -s as a verb does: `grows` and `delays` joined it with `TextArea.growsWithValue` and `Spinner.delaysAppearance`. A name still owed passes by name, from a list there that only shrinks: it leaves the list in the change that renames the prop, and a test fails on a name that is no longer a misfit or that the list did not start with. One name and one polarity for one meaning across specs remain the reader's to check.
...
| `<variant>` | one forced accessibility state, absent in the standard state: `increased-contrast`, `reduce-transparency`, `bold-text` | Apple: all three; web: `reduce-transparency` for each example that renders glass, as Apple records it, and `increased-contrast` for every example on `web-desktop` at `regular` density only |

The platform segment is a platform key and never a stack name, so a column of the gallery is a column of the parity report and a second Apple target (a macOS render beside the iOS one) needs no rename. Today Apple records `ios` (iPhone 17, `swift/Tests/DSSnapshotTests/README.md`) and the web records `web-desktop` and `web-touch` (`web/apps/vrt/playwright.config.ts`); a cell missing on one side is a *missing pair*, and a variant the other stack's matrix does not record, on that platform at that density, is not.
