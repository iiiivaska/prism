// forked from design-sync lib/css.mjs - Prism's token CSS is two files in two directories (src/generated/prism/tokens.css, src/generated/motion.css) beside a second brand that must not ship; cfg.tokensGlob may be a string[]
...
export function copyTokens({ tokensPkg, tokensGlob, nodeModules, out }) {
  ...
  // Prism fork: each pattern of a string[] tokensGlob is collected in order.
  for (const glob of Array.isArray(tokensGlob) ? tokensGlob : tokensGlob ? [tokensGlob] : []) {
