// forked from design-sync lib/dts.mjs - @iiiivaska/prism-react declares its types only under exports['.'].types (no top-level `types`), and its IconName is a 51-member literal union the stock caps turn into `unknown`
...
// Prism fork: a package that declares its types only under `exports['.'].types`.
const exportsTypes = (pj) => {
  const e = pj.exports?.['.'] ?? pj.exports;
  return e && typeof e === 'object' && typeof e.types === 'string' ? e.types : undefined;
};
...
export function findTypesRoot(pkgDir, pkgJson) {
  ...
  const t = pkgJson.types || pkgJson.typings || exportsTypes(pkgJson);
...
function projectFor(pkgDir, typesRoot) {
  ...
  const entry = join(pkgDir, (pubEntry && existsSync(join(pkgDir, pubEntry)) ? pubEntry : null) || pj.types || pj.typings || exportsTypes(pj) || 'index.d.ts');
...
function typeText(t, at) {
  ...
  if (t.isUnion()) {
    ...
    // Prism fork: a union of string literals is kept whole - IconName (51 registry ids) is the vocabulary
    // Icon, Button and Card take, and a literal union is valid TS at any length.
    if (uniq.length > 1 && uniq.every((u) => /^"[^"]*"$/.test(u))) return uniq.join(' | ');
