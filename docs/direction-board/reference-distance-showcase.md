...
*Pointer added 2026-09-26: condition 8 fired at `e56272f`, which took RD-12. `identifier-copy` reads "INV-209316", an invoice number, where it read "B-4417", so both apps show the new label in the pill. On Apple it is also in the props line under it, which the app prints from the catalogue regenerated in that commit. That was read from the source, as SD-10 records; neither app was run for this note. The change fills no slot. So by this condition's own words, [gallery §13](reference-distance-gallery.md#13-re-review-of-the-identifier-chip-after-rd-12s-relabel--2026-09-26) answers it: that section reads the 16 re-recorded images, and none is a copy. §13.3's nearest specimen to `md-with-avatar` is now that invoice number, not the fleet unit number, which moves the page away from the plate chip. This note answers condition 8 for `identifier-copy` only. It leaves SD-10 as it stands.*

*Pointer added 2026-09-26: condition 8 fired again at `8987b2d` (P4-10, ADR-0040), which changed a tone of `IconButton/on-glass-over-map`. In light its circle on the glass tile is the ink solid with a white glyph, where it was the white solid with an ink glyph, wherever either app stages the example; dark draws the same white circle. Its props did not change, so the line the Apple app prints under it reads as before: the catalogue regenerated in that commit moves only Button's and IconButton's spec versions. That was read from the source, as SD-10 records; neither app was run for this note. The change fills no slot. So by this condition's own words, [gallery §15](reference-distance-gallery.md#15-re-review-of-iconbuttons-circle-on-glass-after-p4-10--2026-09-26) answers it: that section reads the 6 re-recorded images, and none is a copy. This note answers condition 8 for `on-glass-over-map` only. It leaves SD-10 as it stands.*
