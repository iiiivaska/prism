...
**No reference image was fetched, screenshotted or stored.** ADR-0015 rule 1 forbids reference imagery in this tree and `pnpm lint:reference-copy` enforces the copy half of it — it passes over this file, and over **the part of the tree the guard scans**, with this file in it (588 files, 143 denylist entries, exit 0, re-run 2026-09-22 after the target was added). That was not the whole tree when this review was written: `SCAN_TARGETS` covered `web/apps/*` but named no `swift/Showcase`, so the web showcase was gated and the Apple one was not, and its 24 files were scanned by hand for this review — **0 findings**. **SD-5** recorded the gap and it was closed the same day: the target is in `SCAN_TARGETS`, the guard now reports **588 files** with the same 0 findings, and CI makes the answer again on every push rather than taking this document's word for it. Reference shots are named here by the id of `references.json` and a neutral nickname, never by a client's brand name (ADR-0015 rule 2), exactly as the board's own review does.
...
**Action — taken the same day, by the orchestrator rather than a later ticket.** `{ dir: "swift/Showcase", required: false }` is now in `SCAN_TARGETS`, with the reason in a comment beside it. The guard reports **588 files** (143 denylist entries, exit 0), up from 564: the 24 Apple showcase files, and **0 findings** — the same answer the hand scan gave, now given by CI on every push. It was done here rather than deferred because the gap is one line and the finding's own argument is that a hand scan clears a moment while a target clears a branch. The file count is updated in this introduction, in `reference-distance-gallery.md` §2 and in the roadmap rows that quote it.
