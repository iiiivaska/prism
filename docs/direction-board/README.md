...
| [**reference-distance-showcase.md**](reference-distance-showcase.md), reviewed 2026-09-22 (roadmap P5-3) | `web/apps/showcase` and `swift/Showcase`, built and run | 7 web and 11 Apple screen shapes | no screen is a copy | SD-1 (Icons is the closest call), SD-2 (the web axis bar), SD-3 (the clearance rests on the chrome not being Prism, and Phase 5 removes that), SD-4 (a dark-scheme chrome defect, not a distance matter), SD-5 (`lint:reference-copy` did not scan `swift/Showcase`, so half of that review's subject had no gate — closed the same day, the target is in `SCAN_TARGETS`) |

**Re-reviewed 2026-09-23.** The table is the state of 2026-09-22. Both companions have since gained dated sections, and neither finds a copy. [Gallery §9](reference-distance-gallery.md#9-re-review-of-the-wave-1-examples--2026-09-23) reviews the wave-1 examples, so the gallery review now covers 1176 images and 67 examples of 8 components (RD-5 to RD-8). [Showcase §11](reference-distance-showcase.md#11-re-review-of-the-icons-screens--2026-09-23) re-reviews the Icons screens of both apps (SD-6; SD-7, fixed the same day; SD-4 closed by P4-D4), and [§12](reference-distance-showcase.md#12-re-review-of-the-component-pages--2026-09-23) the component pages (SD-8 closed, SD-9). **Reviewed again 2026-09-26:** [gallery §10](reference-distance-gallery.md#10-review-of-avatars-examples--2026-09-26) reads Avatar's twelve examples, the 204 images of `5522846`, so the gallery review now covers 1380 images and 79 examples of 9 components. None is a copy (RD-9, RD-10). Later the same day, [gallery §11](reference-distance-gallery.md#11-review-of-chips-examples--2026-09-26) reads Chip's thirteen, the 224 images of `5b1663c`, so the gallery review covers 1604 images and 92 examples of 10 components. None is a copy (RD-11, RD-12). [Showcase §13](reference-distance-showcase.md#13-re-review-of-the-avatar-and-chip-pages--2026-09-26) reads the Avatar and Chip pages of both apps. The web app's are not a copy. The Apple app could not be run in that session, so its Chip page and its `Chip/md-with-avatar` page are not reviewed (SD-10). Then [gallery §12](reference-distance-gallery.md#12-review-of-the-webs-reduce-transparency-images--2026-09-26) reads the 144 web Reduce Transparency images of `29b5490`, new variants of 18 named examples, so the gallery review covers 1748 images. None is a copy, and none moves an earlier verdict. Then [gallery §13](reference-distance-gallery.md#13-re-review-of-the-identifier-chip-after-rd-12s-relabel--2026-09-26) reads `Chip/identifier-copy`'s 16 images, re-recorded after RD-12's relabel to "INV-209316". None is a copy, and RD-12 is closed. Then [gallery §14](reference-distance-gallery.md#14-review-of-the-webs-increase-contrast-images--2026-09-26) reads the 183 web Increase Contrast images of P4-D14, one per example and scheme on `web-desktop` at regular density: new variants of all 92 named examples, so the gallery review covers 1931 images. None is a copy, and none moves an earlier verdict. Then [gallery §15](reference-distance-gallery.md#15-re-review-of-iconbuttons-circle-on-glass-after-p4-10--2026-09-26) reads the 6 images of `IconButton/on-glass-over-map` that P4-10 re-recorded, whose circle on light glass is now the ink solid (ADR-0040). None is a copy, and none moves an earlier verdict (RD-13). The expiry conditions below are the ones in force after those sections.

Together the three covered **every screen that would be public at a release made from this branch** at `deb6632`, which is what [`docs/legal-checkpoint.md`](../legal-checkpoint.md) §5.2 outstanding item 1 (finding **F-7**) asked for. **At `5b1663c`, and at `29b5490` with gallery §12, they cover every screen but two**: the Apple showcase's Chip page and its `Chip/md-with-avatar` page. A release made from either tree is not cleared until the run that showcase SD-10 describes reads them. Neither companion re-opens anything signed here: both inherit the ↗ affordance as grammar knowingly taken from shot 27220417, exactly as item 4 below recorded it.

**Both companion clearances are dated and expire.** These are the conditions in force since 2026-09-23, with those that the gallery's §10.8 and §11.9 and the showcase's §13.8 added on 2026-09-26. Each document states them in full in the section named, and the conditions of 2026-09-22 that they replace stay readable in gallery §8 and showcase §10.

- **The gallery** ([§9.9](reference-distance-gallery.md#99-what-this-clearance-covers-and-when-it-expires), [§10.8](reference-distance-gallery.md#108-what-this-clearance-covers-and-when-it-expires) and [§11.9](reference-distance-gallery.md#119-what-this-clearance-covers-and-when-it-expires)): a pattern example screen lands; P5-1 rebuilds the board screens into the gallery; the gallery holds an example the review does not name (§9.8, §10.7 and §11.8 list the 92 it covers by id, so a new example and every example of a newly baselined component are outside it until a dated section reads them); an example other than `IconButton/with-badge` and `Chip/md-with-avatar` fills a slot with a second component; the notification composition moves towards the reference's bell; a re-record after `deb6632` changes what an example draws; a mark on the map moves towards shot 27220417's live marker, by gaining a heading arrow, a dashed ring or a readout card beside it, or a Chip filter pill is composed over the map with map controls, a glass card or a mark; or an identifier chip moves towards that shot's plate chip, as a one-letter avatar beside a vehicle's number, a chip over a vehicle drawing, in a vehicle card or as a table's row header, or an identifier truncated beside its copy glyph. A forced state the web begins to record adds new variants of named examples, which a dated section reads before a release, as [§12](reference-distance-gallery.md#12-review-of-the-webs-reduce-transparency-images--2026-09-26) reads the 144 Reduce Transparency images.
- **The showcase apps**: Phase 5 rebuilds the chrome from real components, or the web axis bar gains a viewport control or a canvas/docs tab pair ([§10](reference-distance-showcase.md#10-what-this-clearance-does-and-does-not-cover), conditions 1 and 3); for the Icons screens, anything that acts on one entry, a ladder that follows a chosen entry, a new control or controls that differ between the two apps, a tile that loses its registry row or leads with the vendor's name, or the Phase 5 chrome ([§11.8](reference-distance-showcase.md#118-what-the-icons-clearance-covers-and-when-it-expires)); and a staged example other than `with-badge` and `Chip/md-with-avatar` that fills a slot with a second component, a staged example the gallery review does not name, the notification composition moving towards the bell (the apps' own chrome included), either app putting two Prism components together outside an example's stage in a way it did not at `5b1663c` (an Avatar or a Chip in the apps' own chrome included), or a staged example changing what it draws, from the commit that changes it ([§12.7](reference-distance-showcase.md#127-what-this-clearance-covers-and-when-it-expires) and [§13.8](reference-distance-showcase.md#138-what-this-clearance-covers-and-when-it-expires)). On Apple, `md-with-avatar`'s condition stays fired until SD-10's run.

Rule 3 asks for the review before **every** release, so those conditions are read again each time, not once.
