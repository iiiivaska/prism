...
**Both companion clearances are dated and expire.** These are the conditions in force since 2026-09-23. Each document states them in full in the section named, and the conditions of 2026-09-22 that they replace stay readable in gallery §8 and showcase §10.

- **The gallery** ([§9.9](reference-distance-gallery.md#99-what-this-clearance-covers-and-when-it-expires)): a pattern example screen lands; P5-1 rebuilds the board screens into the gallery; the gallery holds an example the review does not name (its §9.8 lists the 67 it covers by id, so a new example and every example of a newly baselined component are outside it until a dated section reads them); an example other than `IconButton/with-badge` fills a slot with a second component; the notification composition moves towards the reference's bell; or a re-record after `deb6632` changes what an example draws.
