...
**Both companion clearances are dated and expire.** These are the conditions in force since 2026-09-23, with the gallery's condition 7 added on 2026-09-26. Each document states them in full in the section named, and the conditions of 2026-09-22 that they replace stay readable in gallery §8 and showcase §10.

- **The gallery** ([§9.9](reference-distance-gallery.md#99-what-this-clearance-covers-and-when-it-expires) and [§10.8](reference-distance-gallery.md#108-what-this-clearance-covers-and-when-it-expires)): a pattern example screen lands; P5-1 rebuilds the board screens into the gallery; the gallery holds an example the review does not name (§9.8 and §10.7 list the 79 it covers by id, so a new example and every example of a newly baselined component are outside it until a dated section reads them); an example other than `IconButton/with-badge` fills a slot with a second component; the notification composition moves towards the reference's bell; a re-record after `deb6632` changes what an example draws; or a mark on the map moves towards shot 27220417's live marker, by gaining a heading arrow, a dashed ring or a readout card beside it.
- **The showcase apps**: Phase 5 rebuilds the chrome from real components, or the web axis bar gains a viewport control or a canvas/docs tab pair ([§10](reference-distance-showcase.md#10-what-this-clearance-does-and-does-not-cover), conditions 1 and 3); for the Icons screens, anything that acts on one entry, a ladder that follows a chosen entry, a new control or controls that differ between the two apps, a tile that loses its registry row or leads with the vendor's name, or the Phase 5 chrome ([§11.8](reference-distance-showcase.md#118-what-the-icons-clearance-covers-and-when-it-expires)); and a staged example other than `with-badge` that fills a slot with a second component, a staged example the gallery review does not name, the notification composition moving towards the bell (the apps' own chrome included), or either app putting two Prism components together outside an example's stage in a way it did not at `deb6632` ([§12.7](reference-distance-showcase.md#127-what-this-clearance-covers-and-when-it-expires)).
