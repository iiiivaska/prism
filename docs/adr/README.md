...
| [0039](0039-the-pressed-solid.md) | A pressed solid is one lightness step from its rest: `bg.fill.inverse-pressed` (`neutral.850` in light, `neutral.200` in dark) and `bg.fill.inverse-media-pressed` (`neutral.200`) are the pressed fills of Button's primary pill and IconButton's primary and selected circles, on every press, so the Reduce Motion press shows | accepted |
| [0040](0040-materials-a-part-keys.md) | On `inverse`, `accent` and light glass every colour-bearing part keys the material or its spec's `materials` says what it draws there, checked by `material/uneven`: parts take the material's foreground, the solid knocks out on the two opaque materials, no part draws a second ground there, and a meter binds no chart role; the solid on the scheme's glass is `bg.fill.inverse`, as ADR-0030 §3.1 says | accepted |
