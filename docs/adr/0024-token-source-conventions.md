...
- Status: accepted (§4.1 amended by [ADR-0020](0020-brand-model.md): `app.prism.alpha` is the one folded key an alias declares; §13.3 manifest CSS names amended by [ADR-0027](0027-font-emission-layout-and-manifest-names.md); §4.1's folded gradient keys, §6's vivid slots and §9.5's permutation count amended by [ADR-0029](0029-direction-board-sign-off.md): `temperature`, two one-temperature slot pairs, 576 permutations; §6's role list and §9.1's platform row amended by [ADR-0030](0030-semantic-roles-from-the-direction-board.md): `type.axis`, and `platform` also owns `sys.type.metric.xl`)
- Date: 2026-09-15
