...
| C-15 | high | `docs/adr/0014-monorepo-and-distribution.md:37` | Choose the version authority, add a 0.x bump policy and a release-workflow ticket, and stamp VERSION, the Swift constant and the skill from one source. *The 0.x diff policy is settled by ADR-0024 §14 (2026-09-15); the rest stays open.* | new |
...
| S-02 | low | `docs/adr/0011-accessibility-tiers-ci.md:51` | Fix the stale paths in the table, remove the ADR-0004:5 note, and update `tools/README.md` and `brand.json`. *The `brand.json` row is settled by ADR-0021 (2026-09-15; the files land in P1-8), and the `tools/tokens/README.md` row was fixed the same day.* | new |
