...
> **Superseded in part by ADR-0023 (2026-09-15).** The extension is `app.prism.spring`, not `dev.prism.spring` (§0 item 4, §5.3, §6.3, §9 item 1). The motion contexts are `default` and a layered `reduced`, not `full | reduced` (§6.3, §9 item 5); the presentation crossfade is the number flag `sys.motion.presentation.crossfade`, not a string token (§6.3); reduced `smooth` is (0.30, 0). The component contract of §6.3 stands, made precise in ADR-0023 §8.4: presentations fade, in-place scale and blur become opacity changes, in-place movement keeps a tightened spring; Motion's `MotionConfig reducedMotion="user"` (§0 item 5, §9 item 5) is not used. CSS `linear()` comes from 1 ms samples of Motion's physics generator simplified by Ramer–Douglas–Peucker, not a uniform sample or a Prism-only sampler (§5.4, §9 item 3). Parity tolerances are ADR-0023 §11, not 1 % / 10 ms / 0.01 at five points (§5.4, §9 item 9). The Swift and CSS samples in §5.4 are superseded by `tools/tokens/ARCHITECTURE.md` §9. Open question 2 is answered; question 3 is moot because the Tokens Studio flavor omits transitions (ARCHITECTURE §9.9); question 4 is a roadmap verify item; question 7 is answered by ARCHITECTURE §15 F24 (`duration-ds-*` utilities compile). The haptics registry is `spec/haptics.yaml` (§7.5).
