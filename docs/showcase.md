...
| `platforms.<key>` is `full`/`adapted`, no manifest entry | "Specified, not implemented here yet", the spec summary, the example ids it will have, the spec link. Every one of the 48 unimplemented specs is a row, so the app never silently omits |
