...
- **sys.color segments**: `accent`, `accent-strong`, `axis`, `band`, `bg`, `border`, `boundary`, `chart`, `comparison`, `comparison-2`, `critical`, `dimmed`, `fill`, `focus`, `ghost`, `glow`, `grid`, `hairline`, `icon`, `info`, `inverse`, `nested`, `neutral`, `now`, `on-accent`, `on-accent-strong`, `on-badge`, `on-glass`, `on-glass-light`, `on-glass-secondary`, `on-glass-tertiary`, `on-inverse`, `on-vivid`, `overlay`, `page`, `plot`, `pressed`, `primary`, `raised`, `secondary`, `series`, `status`, `strong`, `subtle`, `success`, `surface`, `target`, `tertiary`, `text`, `tint`, `warning`. Numeric segments are free. The list is closed in practice: a new segment needs its line here in the same change, and `tools/tokens/docs.test.ts` checks it against the ids (ADR-0024 §13).
- **Brand-owned ref ids**: `ref.color.slot.<light|dark>.<bg-page|bg-fill-accent|text-on-accent|text-accent>` and `ref.font.apple.<ui|display|mono>`, plus the ramps, series, gradients, web font slots, `ref.type.scale` and the radius steps listed in `brands/README.md`. System-owned: `ref.color.smoke.<light|dark>` and every other `ref` id (ADR-0020 §1).

Examples (every path exists; `tools/tokens/docs.test.ts` checks them):

- `color.bg.page`, `color.bg.surface`, `color.bg.surface.raised|nested|overlay`, `color.bg.fill.inverse|accent|accent-strong|critical`, `color.bg.fill.neutral.subtle`, `color.bg.tint.accent|success|warning|critical|info`
- `color.text.primary|secondary|tertiary|dimmed|accent|success|warning|critical|info`, `color.text.on-inverse|on-accent|on-accent-strong|on-vivid|on-glass|on-badge`, `color.text.on-glass-light|on-glass-secondary|on-glass-tertiary` (ADR-0022)
- `color.icon.primary|secondary|accent`, `color.icon.status.success|warning|critical|info`
- `color.border.hairline|strong|focus|boundary`, `color.accent`, `color.accent.pressed|subtle|glow`
- `color.chart.series.1…6`, `color.chart.comparison|target|band|grid|axis|now|ghost|plot`
- `type.display.xl|lg|md`, `type.title.lg|md|sm`, `type.headline`, `type.body.lg|md|sm`, `type.label.lg|md|sm`, `type.caption`, `type.micro`, `type.eyebrow`, `type.metric.xl|lg|md|unit`, `type.data` — composite typography: family slot, size, line height, weight, tracking, figures; weights, figures and Dynamic Type styles follow ADR-0021
...
| `platform` | `apple`, `web`, `watch` | `sys.font.**` (selects the brand's font slots: web → `ref.font.<slot>`, apple and watch → `ref.font.apple.<slot>`) |
...
- **Figures** (ADR-0021 §5): `metric.xl` and `metric.lg` use proportional figures (static heroes); `metric.md` and `data` are tabular; values that change while visible set `numeric: tabular` on any role.
...
7. In `sys/**`, every color is an alias, an alias with `app.prism.alpha`, or pure white or black; every `fontFamily` value aliases a font slot; gradient and radius tokens are aliases; every typography token aliases a `ref.type.*` role; other values may be literals (ADR-0020 §3, ADR-0024 §5.6).
