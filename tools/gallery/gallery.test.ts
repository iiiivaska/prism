...
describe('the repository', () => {
  ...
  test('every image is of the rule, every pair is complete, and nothing is wrong', () => {
    expect(gallery.diagnostics).toEqual([]);
    expect(gallery.counts.missing).toBe(0);
    // P3-3's 268 SwiftUI snapshots and P3-4's 220 web screenshots, plus P4-1's Divider (52 SwiftUI, 48 web) and
    // P4-2's Icon (140 SwiftUI, 88 web): 460 and 356, paired. Each wave-1 component moves this number in the commit
    // that lands its baselines.
    expect(gallery.counts.images).toBe(816);
    expect(gallery.counts.paired).toBe(gallery.counts.cells);
    expect(gallery.platforms).toEqual(['ios', 'web-desktop', 'web-touch']);
    expect(gallery.components.filter((c) => c.recorded.length > 0).map((c) => c.name)).toEqual(['Button', 'Divider', 'Icon', 'Surface', 'Text', 'Card']);
